...
.LC0:										# label for string "Enter the string (all lowrer case): " (for 1st printf statement)
	.string	"Enter the string (all lowrer case): "		
.LC1:										# label for string "%s"	(for scanf statement)
	.string	"%s"
.LC2:										# label for string "Length of the string: %d\n" (for 2nd printf statement)
	.string	"Length of the string: %d\n"
	.align 8								# align to 8 byte boundary
.LC3:										# label for string "The string in descending order: %s\n" (for 3rd printf statement)
...
.LFB0:										
	...
	pushq	%rbp							# save rbp (base pointer) onto stack
...
# asgn1.c:6: {
	...
	xorl	%eax, %eax						# eax <-- 0 (erase canary)
...
# asgn1.c:11:     				len = length(str); // calling length function
	...
	call	length							# call length with rdi as argument 
	movl	%eax, -68(%rbp)					# M[rbp-68] <-- eax, save length of string (len) in stack

# asgn1.c:12:     				printf("Length of the string: %d\n", len);
	movl	-68(%rbp), %eax					# eax <-- M[rbp-68], set eax to length of string (len)
	movl	%eax, %esi						# esi <-- eax, set esi to length of string (len)
...
# asgn1.c:13:     				sort(str, len, dest); // calling sorting function
	...
	movl	-68(%rbp), %ecx					# ecx <-- M[rbp-68], set ecx to length of string (len)
	leaq	-64(%rbp), %rax					# rax <-- M[rbp-64], set rax to address of str
	movl	%ecx, %esi						# esi <-- ecx, set esi to length of string (len)
	movq	%rax, %rdi						# rdi <-- rax, set rdi to address of str
...
.L6:
# asgn1.c:20:     				for (i = 0; str[i] != '\0'; i++) // computing length of the string
	addl	$1, -4(%rbp)					# M[rbp-4]+=1, set i to i+1 (i.e. i++)
.L5:
# asgn1.c:20:     				for (i = 0; str[i] != '\0'; i++) // computing length of the string
...
# asgn1.c:20:     				for (i = 0; str[i] != '\0'; i++) // computing length of the string
	testb	%al, %al						# test if al is 0 or not, if it is not 0 then jump to .L6, i.e. str[i] != '\0'
...
.LFB2:
	...
	movq	%rdi, -24(%rbp)					# M[rbp-24] <-- rdi, set M[rbp-24] to address of str
	movl	%esi, -28(%rbp)					# M[rbp-28] <-- esi, set M[rbp-28] to len
	movq	%rdx, -40(%rbp)					# M[rbp-40] <-- rdx, set M[rbp-40] to address of dest
...
# asgn1.c:34:             		if (str[i] < str[j]) // sorting in ascending order
	...
	jge	.L11								# if al is greater than or equal to dl, then jump to .L11, i.e. if str[i] >= str[j], then jump to L11
...
# asgn1.c:43:     				reverse(str, len, dest);
	movq	-40(%rbp), %rdx					# rdx <-- M[rbp-40], set rdx to dest
	movl	-28(%rbp), %ecx					# ecx <-- M[rbp-28], set ecx to len
	movq	-24(%rbp), %rax					# rax <-- M[rbp-24], set rax to str
	movl	%ecx, %esi						# esi <-- ecx, set esi to len
	movq	%rax, %rdi						# rdi <-- rax, set rdi to str
	call	reverse							# call reverse function with rdi, rsi, rdx as arguments
...
# asgn1.c:51:         			for (j = len - i - 1; j >= len / 2; j--) // reversing the string
	movl	-28(%rbp), %eax					# eax <-- M[rbp-28], set eax to len, i.e. j = len
	subl	-8(%rbp), %eax					# eax <-- eax-8, set eax to len-i, i.e. j = len - i

# asgn1.c:51:         			for (j = len - i - 1; j >= len / 2; j--) // reversing the string
	subl	$1, %eax						# eax <-- eax-1, set j to j-1
...
# asgn1.c:53:             		if (i == j)
	movl	-8(%rbp), %eax					# eax <-- M[rbp-8], set eax to i
	cmpl	-4(%rbp), %eax					# compare eax to M[rbp-4], check difference between values of eax and M[rbp-4], i.e. i and j
...
# asgn1.c:57:                 	temp = str[i];
	...
	movb	%al, -9(%rbp)					# M[rbp-9] <-- al, set temp to str[i]
...
# asgn1.c:49:     				for (i = 0; i < len / 2; i++)
	addl	$1, -8(%rbp)						# M[rbp-8] <-- M[rbp-8] + 1, set i to i+1 (i.e. i++)
.L15:
# asgn1.c:49:     				for (i = 0; i < len / 2; i++)
...
# asgn1.c:49:     				for (i = 0; i < len / 2; i++)
	...
	jl	.L20									# if eax is less than i, jump to .L20
