...
.LC0:										# label for string "Enter the string (all lowrer case): "
	.string	"Enter the string (all lowrer case): "		
.LC1:										# label for string "%s"
	.string	"%s"
...
# asgn1.c:6: {
	...
	movq	%rax, -8(%rbp)					# [rbp-8] save canary in stack
...
# asgn1.c:10:     scanf("%s", str);
	leaq	-64(%rbp), %rax					# rax <-- [rbp-64], set rax to address of str
...
# asgn1.c:11:     len = length(str); // calling length function
	...
	movl	%eax, -68(%rbp)					# [rbp-68] save length of string in stack

# asgn1.c:12:     printf("Length of the string: %d\n", len);
	movl	-68(%rbp), %eax					# eax <-- [rbp-68], set eax to length of string
...
# asgn1.c:13:     sort(str, len, dest); // calling sorting function
	leaq	-32(%rbp), %rdx					# rdx <-- [rbp-32], set rdx to address of dest
	movl	-68(%rbp), %ecx					# ecx <-- [rbp-68], set ecx to length of string
	leaq	-64(%rbp), %rax					# rax <-- [rbp-64], set rax to address of str
...
# asgn1.c:14:     printf("The string in descending order: %s\n", dest);
	leaq	-32(%rbp), %rax					# rax <-- [rbp-32], set rax to address of dest
...
# asgn1.c:16: }
	movq	-8(%rbp), %rcx					# rcx <-- [rbp-8], set rcx to canary
...
.LFB1:
	...
	movq	%rsp, %rbp						# rbp <-- rsp, set base pointer to current stack pointer
	.cfi_def_cfa_register 6					# use register 6 for computing CFA
	movq	%rdi, -24(%rbp)					# [rbp-24] <-- rdi, set [rbp-24] to address of str

# asgn1.c:20:     for (i = 0; str[i] != '\0'; i++) // computing length of the string
	movl	$0, -4(%rbp)					# [rbp-4] <-- 0, set i to 0

# asgn1.c:20:     for (i = 0; str[i] != '\0'; i++) // computing length of the string
	jmp	.L5									# jump to .L5
.L6:
# asgn1.c:20:     for (i = 0; str[i] != '\0'; i++) // computing length of the string
	addl	$1, -4(%rbp)					# [rbp-4]+=1, set i to i+1
.L5:
# asgn1.c:20:     for (i = 0; str[i] != '\0'; i++) // computing length of the string
	movl	-4(%rbp), %eax					# eax <-- [rbp-4], set eax to i
	movslq	%eax, %rdx						# rdx <-- eax, set rdx to i
	movq	-24(%rbp), %rax					# rax <-- [rbp-24], set rax to address of str
	addq	%rdx, %rax						# rax <-- rax+rdx, set rax to address of str+i
	movzbl	(%rax), %eax					# eax <-- [rax], set eax to value at address of str+i
...
# asgn1.c:25: }
	...
	.cfi_endproc							# close the unwind entry previously opened by .cfi_startproc. and emit it to .eh_frame
.LFE1:
	.size	length, .-length				# set size of length function to .-length
...
.LFB2:
	...
	movq	%rdi, -24(%rbp)					# [rbp-24] <-- rdi, set [rbp-24] to address of str
	movl	%esi, -28(%rbp)					# [rbp-28] <-- esi, set [rbp-28] to address of dest
	movq	%rdx, -40(%rbp)					# [rbp-40] <-- rdx, set [rbp-40] to address of len

# asgn1.c:30:     for (i = 0; i < len; i++)
	movl	$0, -8(%rbp)					# [rbp-8] <-- 0, set i to 0

# asgn1.c:30:     for (i = 0; i < len; i++)
	jmp	.L9									# jump to .L9
.L13:
# asgn1.c:32:         for (j = 0; j < len; j++)
	movl	$0, -4(%rbp)					# [rbp-4] <-- 0, set j to 0
# asgn1.c:32:         for (j = 0; j < len; j++)
	jmp	.L10								# jump to .L10
.L12:
# asgn1.c:34:             if (str[i] < str[j]) // sorting in ascending order
	movl	-8(%rbp), %eax					# eax <-- [rbp-8], set eax to i
	movslq	%eax, %rdx						# rdx <-- eax, set rdx to i
	movq	-24(%rbp), %rax					# rax <-- [rbp-24], set rax to address of str
	addq	%rdx, %rax						# rax <-- rax+rdx, set rax to address of str+i
	movzbl	(%rax), %edx					# edx <-- [rax], set edx to value at address of str+i

# asgn1.c:34:             if (str[i] < str[j]) // sorting in ascending order
	movl	-4(%rbp), %eax					# eax <-- [rbp-4], set eax to j
	movslq	%eax, %rcx						# rcx <-- eax, set rcx to j
	movq	-24(%rbp), %rax					# rax <-- [rbp-24], set rax to address of str
	addq	%rcx, %rax						# rax <-- rax+rcx, set rax to address of str+j
	movzbl	(%rax), %eax					# eax <-- [rax], set eax to value at address of str+j
...
# asgn1.c:36:                 temp = str[i];
	movzbl	(%rax), %eax					# eax <-- [rax], set eax to value at address of str+i
	movb	%al, -9(%rbp)					# [rbp-9] <-- al, set [rbp-9] to al
...
# asgn1.c:37:                 str[i] = str[j];
	movl	-8(%rbp), %edx					# edx <-- [rbp-8], set edx to i
	movslq	%edx, %rcx						# rcx <-- edx, set rcx to i
	movq	-24(%rbp), %rdx					# rdx <-- [rbp-24], set rdx to address of str
...
# asgn1.c:38:                 str[j] = temp;
	movb	%al, (%rdx)						# [rdx] <-- al, set [rdx] to al
	movl	-4(%rbp), %eax					# eax <-- [rbp-4], set eax to j
	movslq	%eax, %rdx						# rdx <-- eax, set rdx to j
	movq	-24(%rbp), %rax					# rax <-- [rbp-24], set rax to address of str
...
# asgn1.c:38:                 str[j] = temp;
	movzbl	-9(%rbp), %eax					# eax <-- [rbp-9], set eax to value at address of str+j
	movb	%al, (%rdx)						# [rdx] <-- al, set [rdx] to al
.L11:
# asgn1.c:32:         for (j = 0; j < len; j++)
	addl	$1, -4(%rbp)					# [rbp-4] <-- [rbp-4]+1, set j to j+1
.L10:
# asgn1.c:32:         for (j = 0; j < len; j++)
	movl	-4(%rbp), %eax					# eax <-- [rbp-4], set eax to j
...
# asgn1.c:43:     reverse(str, len, dest);
	...
	movq	-40(%rbp), %rdx					# rdx <-- [rbp-40], set rdx to len
	movl	-28(%rbp), %ecx					# ecx <-- [rbp-28], set ecx to dest
	movq	-24(%rbp), %rax					# rax <-- [rbp-24], set rax to str
...
.LFB3:
	...
	movq	%rdi, -24(%rbp)					# [rbp-24] <-- rdi, set [rbp-24] to address of str
	movl	%esi, -28(%rbp)					# [rbp-28] <-- esi, set [rbp-28] to address of dest
	movq	%rdx, -40(%rbp)					# [rbp-40] <-- rdx, set [rbp-40] to len
...
# asgn1.c:51:         for (j = len - i - 1; j >= len / 2; j--) // reversing the string
	movl	-28(%rbp), %eax					# eax <-- [rbp-28], set eax to address of dest
	subl	-8(%rbp), %eax					# eax <-- eax-8, set eax to address of dest-i

# asgn1.c:51:         for (j = len - i - 1; j >= len / 2; j--) // reversing the string
	subl	$1, %eax						# eax <-- eax-1, set eax to address of dest-i-1
	movl	%eax, -4(%rbp)					# [rbp-4] <-- eax, set [rbp-4] to address of dest-i-1
...
# asgn1.c:51:         for (j = len - i - 1; j >= len / 2; j--) // reversing the string
	cmpl	%eax, -4(%rbp)					# compare eax to [rbp-4], set flags if eax is less than [rbp-4]			
	jl	.L18								# jump to .L18 if eax is less than [rbp-4]	

# asgn1.c:53:             if (i == j)
	movl	-8(%rbp), %eax					# eax <-- [rbp-8], set eax to i
	cmpl	-4(%rbp), %eax					# compare eax to [rbp-4], set flags if i is less than j
...
# asgn1.c:57:                 temp = str[i];
	movzbl	(%rax), %eax					# eax <-- [rax], set eax to str[i]
	movb	%al, -9(%rbp)					# [rbp-9] <-- al, set [rbp-9] to str[i]
...
# asgn1.c:58:                 str[i] = str[j];
	movzbl	(%rax), %eax					# eax <-- [rax], set eax to str[j]
# asgn1.c:58:                 str[i] = str[j];
	movb	%al, (%rdx)						# [rdx] <-- al, set [rdx] to str[j]
...
# asgn1.c:59:                 str[j] = temp;
	movzbl	-9(%rbp), %eax					# eax <-- [rbp-9], set eax to temp
	movb	%al, (%rdx)						# [rdx] <-- al, set [rdx] to temp
...
# asgn1.c:49:     for (i = 0; i < len / 2; i++)
	addl	$1, -8(%rbp)						# [rbp-8] <-- [rbp-8] + 1, set i to i+1
.L15:
# asgn1.c:49:     for (i = 0; i < len / 2; i++)
	movl	-28(%rbp), %eax						# eax <-- [rbp-28], set eax to len
...
# asgn1.c:65:         dest[i] = str[i];
	movl	-8(%rbp), %edx						# edx <-- i, set edx to i
	movslq	%edx, %rcx							# rcx <-- edx, set rcx to i
...
# asgn1.c:65:         dest[i] = str[i];
	movzbl	(%rax), %eax						# eax <-- [rax], set eax to str[i]
# asgn1.c:65:         dest[i] = str[i];
	movb	%al, (%rdx)							# [rdx] <-- al, set [rdx] to str[i]
# asgn1.c:64:     for (i = 0; i < len; i++)
	addl	$1, -8(%rbp)						# [rbp-8] <-- [rbp-8] + 1, set i to i+1
.L21:
# asgn1.c:64:     for (i = 0; i < len; i++)
