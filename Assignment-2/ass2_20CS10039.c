...
int readInt(int *n){
    ...
    for(int i = 1; i < len-1; i++){
        if(buffer[i] < '0' || buffer[i] > '9'){                                   // if the character is not a digit, return ERR
            return ERR;
        }
    }

    int i = 0;                                                              
    if(buffer[0] == '-'){                                                      // if the first character of the string is '-', return 0
...
int printInt(int n){
    ...
    while(n){                                                               // continue till the number is not zero
        buffer[len++]=n%10+'0';                                               // add unit digit to the string
...
int readFlt(float *f){
    ...
    for(int i = 1; i < len-1; i++){
        if(buffer[i]=='.'){                                                    // if the character is '.', increment the count
            count_decimal_chars++;
        }
        if((buffer[i] < '0' || buffer[i] > '9') && buffer[i]!='.' && count_decimal_chars>1){      // if the character is not a digit or number of decimal characters > 1, return ERR
            return ERR;
        }
    }
